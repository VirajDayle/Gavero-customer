import { MenuSection } from "./type";

export const RESTAURANT_SECTIONS: MenuSection[] = [
  {
    id: "1",
    name: "Burgers",
    type: "flat-section",
    data: [
      {
        id: "burger-1",
        name: "Classic Veg Burger",
        description: "Fresh veggie patty with lettuce and mayo",
        price: 149,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
        image:
          "data:image/jpeg;base64,",
      },
      {
        id: "burger-2",
        name: "Cheese Burst Burger",
        description: "Loaded with melted cheese",
        price: 199,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "burger-3",
        name: "Chicken Zinger Burger",
        description: "Crispy chicken fillet with spicy sauce",
        price: 249,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "burger-4",
        name: "Aloo Tikki Delight",
        description: "Crispy potato patty with local spices",
        price: 99,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "burger-5",
        name: "BBQ Bacon Burger",
        description: "Smoked bacon, beef patty, BBQ sauce",
        price: 349,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "burger-6",
        name: "Mushroom Swiss Burger",
        description: "Sautéed mushrooms with swiss cheese",
        price: 229,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "burger-7",
        name: "Spicy Paneer Burger",
        description: "Crispy fried paneer patty with peri-peri mayo",
        price: 189,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "burger-8",
        name: "Double Grilled Chicken Burger",
        description: "Two flame-grilled chicken patties",
        price: 299,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "burger-9",
        name: "Fish Fillet Burger",
        description: "Crispy fish patty with tartar sauce",
        price: 279,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: false,
      },
      {
        id: "burger-10",
        name: "Vegan Avocado Burger",
        description: "Plant-based patty with fresh avocado smash",
        price: 259,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
    ],
  },

  {
    id: "2",
    name: "Pizza",
    type: "nested-section",
    data: [
      {
        id: "veg-pizza",
        name: "Vegetarian Pizza",
        type: "sub-section",
        data: [
          {
            id: "pizza-1",
            name: "Margherita",
            description: "Classic cheese pizza",
            price: 299,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pizza-2",
            name: "Farmhouse",
            description: "Loaded with fresh vegetables",
            price: 399,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pizza-5",
            name: "Paneer Tikka Pizza",
            description: "Spiced paneer, onions, and capsicum",
            price: 449,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pizza-6",
            name: "Four Cheese Pizza",
            description: "Mozzarella, Cheddar, Gouda, and Blue cheese",
            price: 499,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pizza-7",
            name: "Corn & Mushroom Pizza",
            description: "Sweet corn and earthy mushrooms",
            price: 379,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pizza-8",
            name: "Peri Peri Veg Pizza",
            description: "Spicy peri peri sauce with mixed vegetables",
            price: 419,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pizza-9",
            name: "Jalapeno Popper Pizza",
            description: "Fiery jalapenos with cream cheese drizzle",
            price: 429,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
        ],
      },
      {
        id: "nonveg-pizza",
        name: "Non-Vegetarian Pizza",
        type: "sub-section",
        data: [
          {
            id: "pizza-3",
            name: "Chicken Dominator",
            description: "Loaded with chicken toppings",
            price: 499,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pizza-4",
            name: "Pepperoni Pizza",
            description: "Classic pepperoni slices",
            price: 549,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: false,
          },
          {
            id: "pizza-10",
            name: "BBQ Chicken Pizza",
            description: "Smoked BBQ chicken with onions",
            price: 479,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pizza-11",
            name: "Chicken Tikka Pizza",
            description: "Tandoori chicken chunks with mint mayo",
            price: 489,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pizza-12",
            name: "Fiery Hot Meatball Pizza",
            description: "Spicy chicken meatballs and red paprika",
            price: 519,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pizza-13",
            name: "Keema Do Pyaza Pizza",
            description: "Spiced minced mutton with caramelized onions",
            price: 599,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pizza-14",
            name: "Hawaiian Chicken Pizza",
            description: "Chicken ham and pineapple slices",
            price: 469,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
        ],
      },
    ],
  },

  {
    id: "3",
    name: "Beverages",
    type: "nested-section",
    data: [
      {
        id: "cold-drinks",
        name: "Cold Beverages",
        type: "sub-section",
        data: [
          {
            id: "drink-1",
            name: "Coca Cola",
            price: 59,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-2",
            name: "Cold Coffee",
            description: "Chilled coffee with ice cream",
            price: 129,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-3",
            name: "Fresh Lime Soda",
            price: 89,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-4",
            name: "Sprite",
            price: 59,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-5",
            name: "Iced Peach Tea",
            price: 119,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-6",
            name: "Virgin Mojito",
            description: "Mint, lime, and sparkling water",
            price: 149,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-7",
            name: "Mango Shake",
            price: 139,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-8",
            name: "Oreo Milkshake",
            description: "Thick shake blended with Oreo cookies",
            price: 169,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-9",
            name: "Strawberry Smoothie",
            price: 159,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-10",
            name: "Chocolate Frappe",
            price: 179,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-11",
            name: "Red Bull",
            price: 149,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-12",
            name: "Mineral Water",
            price: 30,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
        ],
      },
      {
        id: "hot-drinks",
        name: "Hot Beverages",
        type: "sub-section",
        data: [
          {
            id: "drink-13",
            name: "Masala Chai",
            description: "Brewed with aromatic Indian spices",
            price: 49,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-14",
            name: "Hot Cappuccino",
            price: 119,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-15",
            name: "Cafe Latte",
            price: 129,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-16",
            name: "Espresso Shot",
            price: 89,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-17",
            name: "Hot Chocolate",
            description: "Rich, creamy melted chocolate with milk",
            price: 159,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "drink-18",
            name: "Green Tea",
            price: 69,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
        ],
      },
    ],
  },

  {
    id: "4",
    name: "Desserts",
    type: "nested-section",
    data: [
      {
        id: "cakes",
        name: "Cakes & Pastries",
        type: "sub-section",
        data: [
          {
            id: "cake-1",
            name: "Chocolate Truffle",
            price: 199,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-2",
            name: "Red Velvet Cake",
            price: 249,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-3",
            name: "New York Cheesecake",
            price: 279,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-4",
            name: "Black Forest Pastry",
            price: 129,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-5",
            name: "Pineapple Cake Slices",
            price: 99,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-6",
            name: "Blueberry Muffin",
            price: 119,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-7",
            name: "Sizzling Brownie",
            description: "Served hot with vanilla ice cream",
            price: 219,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "cake-8",
            name: "Apple Pie",
            price: 159,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: false,
          },
        ],
      },
      {
        id: "ice-creams",
        name: "Ice Creams & Waffles",
        type: "sub-section",
        data: [
          {
            id: "ice-1",
            name: "Vanilla Scoop",
            price: 79,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "ice-2",
            name: "Chocolate Brownie Sundae",
            price: 179,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "ice-3",
            name: "Mango Delight Scoop",
            price: 89,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "ice-4",
            name: "Nutella Waffle",
            description: "Warm waffle loaded with Nutella",
            price: 199,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "ice-5",
            name: "Belgian Chocolate Waffle",
            price: 209,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "ice-6",
            name: "Butterscotch Gold",
            price: 99,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "ice-7",
            name: "Tutty Fruity Sundae",
            price: 169,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
        ],
      },
    ],
  },

  {
    id: "5",
    name: "Sides and Starters",
    type: "flat-section",
    data: [
      {
        id: "side-1",
        name: "Classic French Fries",
        description: "Salted golden potato fries",
        price: 119,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-2",
        name: "Peri Peri Fries",
        description: "Spicy peri peri seasoned fries",
        price: 139,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-3",
        name: "Garlic Breadsticks",
        description: "Baked bread brushed with garlic butter",
        price: 149,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-4",
        name: "Cheese Garlic Bread",
        description: "Garlic bread topped with melted mozzarella",
        price: 189,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-5",
        name: "Veg Pizza Pockets",
        price: 129,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-6",
        name: "Chicken Nuggets",
        description: "Crispy fried chicken bites",
        price: 199,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "side-7",
        name: "Spicy Chicken Wings",
        description: "Glazed in hot buffalo sauce",
        price: 249,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "side-8",
        name: "Paneer Popcorn",
        description: "Bite-sized crispy paneer nuggets",
        price: 169,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-9",
        name: "Onion Rings",
        description: "Batter-fried crunchy onion rings",
        price: 109,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-10",
        name: "Mozzarella Cheese Sticks",
        price: 179,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: false,
      },
      {
        id: "side-11",
        name: "Potato Wedges",
        price: 129,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "side-12",
        name: "Crispy Corn Pepper Fry",
        price: 159,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
    ],
  },

  {
    id: "6",
    name: "Pastas and Mains",
    type: "nested-section",
    data: [
      {
        id: "veg-pastas",
        name: "Vegetarian Pasta",
        type: "sub-section",
        data: [
          {
            id: "pasta-1",
            name: "Arrabiata Red Sauce Pasta",
            description: "Spicy tomato sauce with penne",
            price: 249,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pasta-2",
            name: "Alfredo White Sauce Pasta",
            description: "Creamy cheese sauce with mushrooms",
            price: 279,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pasta-3",
            name: "Pink Sauce Pasta",
            description: "Perfect blend of red and white sauces",
            price: 289,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pasta-4",
            name: "Pesto Basil Pasta",
            description: "Penne tossed in fragrant green pesto",
            price: 299,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
          {
            id: "pasta-5",
            name: "Mac and Cheese",
            description: "Classic oven-baked cheesy macaroni",
            price: 269,
            currency: "₹",
            dietaryType: "VEG",
            isAvailable: true,
          },
        ],
      },
      {
        id: "nonveg-pastas",
        name: "Non-Vegetarian Pasta",
        type: "sub-section",
        data: [
          {
            id: "pasta-6",
            name: "Chicken Alfredo Pasta",
            description: "Creamy white sauce with grilled chicken",
            price: 329,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pasta-7",
            name: "Spaghetti Bolognese",
            description: "Spaghetti with minced chicken cooked in tomato sauce",
            price: 349,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pasta-8",
            name: "Spicy Creamy Carbonara",
            price: 359,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: false,
          },
          {
            id: "pasta-9",
            name: "Peri Peri Chicken Pasta",
            price: 339,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
          {
            id: "pasta-10",
            name: "Shrimp Scampi Pasta",
            price: 429,
            currency: "₹",
            dietaryType: "NON_VEG",
            isAvailable: true,
          },
        ],
      },
    ],
  },

  {
    id: "7",
    name: "Sandwiches and Wraps",
    type: "flat-section",
    data: [
      {
        id: "wrap-1",
        name: "Veg Club Sandwich",
        description: "Triple-layered classic veg sandwich",
        price: 149,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "wrap-2",
        name: "Grilled Corn & Cheese Sandwich",
        price: 129,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "wrap-3",
        name: "Paneer Tikka Wrap",
        description: "Spiced paneer wrapped in a soft tortilla",
        price: 179,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "wrap-4",
        name: "Crispy Chicken Wrap",
        description: "Fried chicken strips with lettuce and garlic aioli",
        price: 199,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "wrap-5",
        name: "Chicken Club Sandwich",
        price: 189,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "wrap-6",
        name: "Spicy Egg & Mayo Sandwich",
        price: 119,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "wrap-7",
        name: "Tandoori Chicken Wrap",
        price: 209,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "wrap-8",
        name: "Mexican Bean & Avocado Wrap",
        price: 169,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
    ],
  },

  {
    id: "8",
    name: "Salads and Healthy Bowls",
    type: "flat-section",
    data: [
      {
        id: "salad-1",
        name: "Caesar Salad",
        description: "Fresh romaine lettuce, croutons, parmesan dressing",
        price: 199,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "salad-2",
        name: "Grilled Chicken Salad",
        description: "Healthy greens topped with grilled chicken breast",
        price: 249,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
      {
        id: "salad-3",
        name: "Greek Salad",
        description: "Cucumbers, tomatoes, olives, and feta cheese",
        price: 219,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "salad-4",
        name: "Quinoa Avocado Bowl",
        description: "High protein quinoa with fresh veggies",
        price: 279,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "salad-5",
        name: "Tofu Protein Bowl",
        price: 239,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "salad-6",
        name: "Fruit Salad with Honey Drizzle",
        price: 149,
        currency: "₹",
        dietaryType: "VEG",
        isAvailable: true,
      },
      {
        id: "salad-7",
        name: "Smoked Salmon Salad",
        price: 349,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: false,
      },
      {
        id: "salad-8",
        name: "Keto Chicken Bowl",
        price: 269,
        currency: "₹",
        dietaryType: "NON_VEG",
        isAvailable: true,
      },
    ],
  },
];
